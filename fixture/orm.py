...
class ORMFixture:
    ...
    def convert_contacts_to_model(self, contacts):
        def convert(contact):
            return Contact(id=str(contact.id), firstname=contact.firstname, lastname=contact.lastname,
                           email=contact.email, email2=contact.email2, email3=contact.email3, address=contact.address,
                           mobile_phone=contact.mobile, home_phone=contact.home, work_phone=contact.work, secondary_phone=contact.secondary_phone)
    ...
    def get_contacts_in_group(self, group):
        orm_group = list(
            select(g for g in ORMFixture.ORMGroup if g.id == group.id))[0]
